  ...
  return (
    <Card className="border-none shadow-sm">
      <CardHeader className="p-4 pb-2">
        <CardTitle className="text-[10px] font-bold uppercase">Atividade Recente</CardTitle>
      </CardHeader>
        ...
        {events.map(ev => (
          <div key={ev.id} className="flex gap-3 items-start">
            ...
            <div className="flex flex-col">
              <span className="text-[11px] leading-tight text-foreground font-medium">{ev.text}</span>
              <span className="text-[9px] text-muted-foreground">{formatDistanceToNow(ev.time, { locale: ptBR, addSuffix: true })}</span>
            </div>
          </div>
        ))}
      </CardContent>
